...
function sanitize() // lol I'm such a hacker
{
    ...
    let lasttask = setTimeout(function () {
    }, 0);
...
function makeimage() {
    ...
    if (cfg_direct === true) {
        unsafeWindow.location.href = i.src;
        return false;
    }
    if (cfg_bgclr) {
    ...
    parent.postMessage({src: isrc, url: window.location.href}, "*");
...
function find_text_in_scripts(text, stopword, start_from_top, search_after_word) {
    ...
    for (let c = 0; c < s.length; c++) {
        if (search_after_word && s[c].innerHTML.indexOf(search_after_word) != -1) {
            s[c].innerHTML = s[c].innerHTML.substring(0, s[c].innerHTML.indexOf(search_after_word));
        }
...
function makeworld() {
    ...
    switch (host) {
        ...
        case "deviantart.com":
            ...
            {
                i = q('a[download]');
                if (i && i.href.indexOf("deviantart.com/users/outgoing?") == -1) {
                    i.src = i.href;
    ...
    } else // try again
    {
        ...
        tb = setTimeout(function () {
            console.warn("Didnt find image, waited " + timeout + " ms to try again. page: " + window.location.href);
            tb = 0;
            timeout *= 2;
            i = 0;
            makeworld();
...
function autoresize() {
    if (i.naturalWidth) {
        ...
        rescaled = true;
        rescale(0); // to original size in pixels
    ...
    if (typeof GM.setValue !== "undefined") {
        function saveCfg() {
            ...
            if (q("#hji_cfg_2_bgclr").value) {
                document.body.bgColor = q("#hji_cfg_2_bgclr").value;
            } else {
